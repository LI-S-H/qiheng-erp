...
- 出入库流水号由后端在业务流程生成凭证时统一创建，并由 `uk_stock_bill_no` 唯一索引兜底；查询页面不提供人工创建或修改流水入口。
- `DRAFT` 和 `CANCELLED` 流水不改变库存余额；只有 `CONFIRMED` 流水在事务内更新 `warehouse_stock`，并记录每条明细的 `before_qty`、`change_qty`、`after_qty`。
