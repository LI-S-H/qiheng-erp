...
- 库存调整页面不新增独立业务表，固定查询 `stock_bill.entry_mode=MANUAL_ADJUSTMENT`，并允许按调整流水号、调整单号、仓库、`ADJUST_IN/ADJUST_OUT` 调整方向和状态组合查询。
- 可以在出入库记录页面新增调整入库、调整出库，或补录遗漏的采购入库、销售出库、采购退货、销售退货草稿；补录记录显示明确标识、原业务单号、原因和负责人。
- 可以在库存调整页面新增、编辑、确认和取消调整草稿；该页面不得显示来源生成凭证或人工补录凭证。
