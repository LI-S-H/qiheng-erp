...
- 仓库编码由用户在创建仓库时填写，去除首尾空格后统一按大写保存并保证唯一；仓库编码创建后不可修改，避免 `warehouse_stock.warehouse_code` 冗余字段发生大范围级联更新。
...
- 可以新增、编辑、启用和停用仓库；仓库编码创建后保持只读。
- 重复仓库编码返回 `409 Conflict`，字段长度和格式错误返回 `400 Bad Request`。
