...
CREATE TABLE IF NOT EXISTS warehouse_stock (
    ...
    stock_qty DECIMAL(18,4) NOT NULL DEFAULT 0.0000 COMMENT '当前库存数量',
    locked_qty DECIMAL(18,4) NOT NULL DEFAULT 0.0000 COMMENT '锁定库存数量',
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (id),
...
CREATE TABLE IF NOT EXISTS stock_bill_item (
    ...
    product_name VARCHAR(200) NOT NULL COMMENT '产品名称冗余',
    unit_name VARCHAR(32) NOT NULL DEFAULT '件' COMMENT '单位名称冗余',
    quantity DECIMAL(18,4) NOT NULL DEFAULT 0.0000 COMMENT '本次出入库数量，正数',
    qualified_qty DECIMAL(18,4) NOT NULL DEFAULT 0.0000 COMMENT '合格数量',
    defective_qty DECIMAL(18,4) NOT NULL DEFAULT 0.0000 COMMENT '不合格数量',
    before_qty DECIMAL(18,4) NOT NULL DEFAULT 0.0000 COMMENT '变动前库存',
    change_qty DECIMAL(18,4) NOT NULL DEFAULT 0.0000 COMMENT '库存变动数量，入库为正，出库为负',
    after_qty DECIMAL(18,4) NOT NULL DEFAULT 0.0000 COMMENT '变动后库存',
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    remark VARCHAR(500) NOT NULL DEFAULT '' COMMENT '备注',
    PRIMARY KEY (id),
