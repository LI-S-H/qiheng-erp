...
本文面向面试表达，说明本项目传统 ERP 主体为什么采用模块化单体、MVP 极简表设计、出入库流水、适度冗余和受控业务服务，而不是一开始做复杂的全量 ERP 架构。
...
| 目标 | 说明 |
|---|---|
| 业务闭环 | 产品、仓库、采购、销售、库存能完整跑通 |
| 数据一致 | 采购入库、销售出库、库存数量、出入库流水一致 |
| 可扩展 | MVP 简单，但后续能扩展权限、批次、退货、AI 采购建议 |
...
| `erp-warehouse` | 仓库、库存余额、出入库流水 | 不决定采购价格和销售价格 |
...
| 仓库库存 | `warehouse`、`warehouse_stock`、`stock_bill`、`stock_bill_item` | 出入库流水直接承担库存变动凭证，不再单独建库存流水表 |
| 采购 | `supplier`、`supplier_product`、`purchase_order`、`purchase_order_item` | 供应商、供货产品、采购订单和采购明细 |
| 销售 | `customer`、`sales_order`、`sales_order_item` | 客户、销售订单和销售明细，退货后续复用出入库流水 |
...
| 库存流水表 | 不单独做，出入库流水明细承担凭证 | 后续如账务要求更高再拆库存台账 |
...
库存是 ERP 最容易出错的部分，所以设计上分成两类数据。

| 表 | 作用 | 特点 |
|---|---|---|
| `warehouse_stock` | 当前库存余额 | 查询快，按仓库 + 产品唯一 |
| `stock_bill` | 出入库流水主表 | 记录来源、类型、仓库、确认状态 |
| `stock_bill_item` | 出入库流水明细 | 记录产品、变动前、变动数量、变动后 |

库存更新流程：

```mermaid
flowchart LR
    A["采购单 / 销售单 / 退货单 / 调整单"] --> B["生成出入库流水"]
    B --> C["确认出入库"]
...
因为 MVP 阶段如果同时有“出入库单”和“库存流水表”，容易出现两套记录重复：
...
为了减少复杂度，本项目把 **出入库流水作为库存变动凭证**，把变动前后库存记录在明细表里。这样既能查来源单据，也能查库存变化。
...
- 库存改动入口统一。
- 采购、销售和库存可以互相追溯。
- 后续销售退货、采购退货能复用同一套出入库流水。
- AI 查询库存时只需要读取库存余额和出入库流水，不需要理解所有业务表。
...
| `warehouse_name` | 库存、出入库流水 | 列表查询减少联表 |
...
| 阶段 | 扩展点 | 说明 |
|---|---|---|
| MVP | 产品、仓库、采购、销售、库存主流程 | 先跑通交易闭环 |
| 阶段 2 | 细粒度权限、菜单权限、数据权限 | 增强企业内部管控 |
| 阶段 3 | 销售退货、采购退货、库存调整 | 完善库存业务闭环 |
...
>
> 数据库设计上我没有一开始铺满所有 ERP 表，而是做 MVP 极简设计。比如权限先用角色权限码，不做菜单和按钮权限；产品先不拆 SPU/SKU；仓库先不做库位、批次和序列号。库存部分我重点设计了 `warehouse_stock` 和 `stock_bill / stock_bill_item`，用库存余额表负责快速查询，用出入库流水负责库存变动凭证和溯源。
>
