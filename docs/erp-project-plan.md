...
MVP 阶段销售模块不单独设计销售出库单表；销售订单审核后生成仓库模块的 `SALES_OUT` 待确认出库单，不直接扣减库存。仓库人员确认本次出库数量后完成出库、扣减库存、释放锁定库存、生成库存流水并回写销售订单已出库数量。销售退货后续先复用仓库模块 `SALES_RETURN` 入库单，如退货流程复杂再补销售退货单表。
