...
├── erp-system              用户、角色、部门
├── erp-product             产品、分类
├── erp-warehouse           仓库、库存、入库单、出库单、库存流水
...
MVP 阶段暂不设计库位表、批次、序列号、保质期、库存预警表和财务库存台账表；库存先按仓库 + 产品维度维护。采购入库、销售出库、退货和库存调整先生成入库单或出库单，仓库人员确认本次数量后才更新库存并生成库存流水；库存流水明细记录合格数量、不合格数量、变动前库存、变动数量和变动后库存，并能追溯到来源采购单、销售单或退货单。
...
MVP 阶段销售模块不单独设计销售出库单表；销售订单审核后生成仓库模块的 `SALES_OUT` 待确认出库单，不直接扣减库存。仓库人员确认本次出库数量后完成出库、扣减库存、释放锁定库存、生成库存流水并回写销售订单已出库数量。销售退货由独立 `erp-return` 模块承接：审核后生成 `SALES_RETURN` 待确认入库单，确认入库后回写销售退货单及退货明细的处理进度；来源销售明细只提供已出库数量快照和可退校验依据。

### 6.8 erp-return

统一退货模块，负责采购退货和销售退货的申请、审核、仓储作业生成与进度回写。

主要内容：

- 统一退货单和退货明细。
- 基于已入库采购明细的采购退货，审核后生成 `PURCHASE_RETURN` 待确认出库单。
- 基于已出库销售明细的销售退货，审核后生成 `SALES_RETURN` 待确认入库单。
- 退货数量占用、来源数量校验、仓储确认回写及取消时的工作单撤销。
...
PythonTool 不直接绕过业务 Service 查库。Python 分析所需的销售历史、库存、入库单、出库单和库存流水等业务数据，必须先由标准 Workflow 调用基础查询 Tool，再经过权限校验和 ERP Service 获取。PythonTool 只接收经过授权和清洗后的结构化数据，输出分析结果。
...
MVP 阶段暂不单独设计通用 `audit_log` 表，避免在第一版增加过多横切表。普通业务先通过订单状态、操作人字段、确认时间、入库单、出库单和库存流水追溯关键动作。
...
| 敏感数据泄漏 | 字段脱敏 + AI 审计 |
| 库存、采购、销售数据不一致 | 通过业务事务、入库单、出库单、库存流水和审计日志追踪 |
...
```text
用户可以登录系统，维护产品和仓库，完成采购、销售和退货流程。
系统可以正确更新库存，并通过入库单、出库单和库存流水追踪作业与库存变化。
用户可以通过普通接口查询数据。
