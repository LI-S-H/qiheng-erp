...
- 入库单和出库单都必须展示业务对象，列表列名统一为“业务对象”，不能使用泛化“往来方”。采购相关单据显示供应商、销售相关单据显示客户，库存调整单显示来源仓库（取发生调整仓库的名称快照）；详情和编辑态中，调整单字段名明确为“来源仓库”。
- 列表采用“单据父行 + 商品明细区”的主从表格：父行只按入库单/出库单主表字段拆成独立列，展示单号、类型、录入方式、来源类型、来源单号、业务对象、仓库、状态、负责人、创建时间和操作；不得把“类型/来源”“业务对象/仓库”“状态/操作”等不同字段混在同一列。字段较多时通过横向滚动承载，不压缩为组合文案。
...
| 业务对象/来源仓库 | `source_party_id`、`source_party_name`；库存调整时保存发生调整仓库的 ID 和名称快照 |
