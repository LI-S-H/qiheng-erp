...
- 业务待办：主卡展示优先级最高的前 8 条，包含采购单待审核、销售单待审核、待确认入库、待确认出库，以及后续接入的库存异常、库存扣减失败、外部同步异常、价格偏离、客户信用、AI 建议等需要人工审核、人工复核或人工修复的事项；点击待办项或“详情”在同页弹窗展示接口返回的全部待办。单据状态类待办只展示待处理数量和“处理对应业务后自动完成”，按钮为“前往完成”并跳转对应业务页面；异常/人工修复类待办展示来源单号、错误信息、发生时间和“完成处理”；价格偏离、客户信用等复核类待办展示产品/客户证据和关键指标。
...
| 业务待办 | `todos[]` | object[] | 单据状态机、价格/信用复核、异常事件和人工任务聚合 | 否 | 空列表 | `businessType` 为可扩展事件源编码，`businessLabel` 为展示标签；`sortWeight` 由后端计算排序，`sourceMode` 区分动态聚合和持久化任务，`completionMode` 区分自动完成和手动完成；`evidence[]` 提供价格/信用等复核依据 |
...
- 待办处理建议采用“动态聚合 + 持久化异常任务”的混合模型：采购单待审核、销售单待审核、待确认入库、待确认出库、库存低于安全线等状态类待办使用 `sourceMode=AGGREGATED`、`completionMode=AUTO`，详情中不铺开大量单据号，只显示待处理数量、完成条件和“前往完成”入口，对应业务状态完成后在下一次 `/dashboard/overview` 中自动消失或数量变化；库存扣减失败、库存补偿失败、外部同步异常、AI 建议待确认等需要人工修复或人工复核的事件使用 `sourceMode=PERSISTED`、`completionMode=MANUAL`，建议后端新建待办任务表保存 `todoId`、事件源、来源单号、错误码、错误信息、处理建议、状态、处理人、处理时间和备注，再通过 `POST /dashboard/todos/{todoId}/complete` 完成处理。
