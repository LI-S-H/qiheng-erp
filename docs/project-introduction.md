...
本项目是一个面向**中小型商贸、批发、经销及仓配类企业**的进销存 ERP 系统，围绕产品、供应商、客户、采购、销售、退货、仓库和库存等核心业务，建设一套可落地、可扩展、可审计的企业内部管理平台。
...
| 核心场景 | 产品管理、采购管理、销售管理、统一退货管理、仓库库存管理、权限管理、AI 查询和分析建议 |
| 建设策略 | 先跑通采购、销售、退货、库存主链路，再逐步扩展复杂能力 |
...
| 仓库人员 | 管理仓库、库存余额、入库、出库和库存调整 | 仓库管理、库存查询、入库单、出库单、库存流水、库存调整 |
...
- 支持客户资料管理。
- 支持采购订单创建、审核、采购入库及采购退货出库流程。
- 支持销售订单创建、审核、库存锁定、销售出库及销售退货入库流程。
- 支持统一退货单的草稿、提交、审核、取消、仓储执行与退货处理进度回写。
- 支持仓库管理、库存余额查询、库存调整、入库单、出库单和库存流水追溯。
- 支持用户、角色、部门和粗粒度权限码管理。
...
| 采购模块 | 供应商、供货产品、采购订单、采购入库、采购退货来源 | 暂不做复杂审批流、退款和换货结算 |
| 销售模块 | 客户、销售订单、库存锁定、销售出库、销售退货来源 | 暂不做复杂价格体系、退款和换货结算 |
| 退货模块 | 统一退货单、采购退货出库、销售退货入库、进度回写 | 暂不做退款、质检、换货和结算流程 |
| 仓库库存模块 | 仓库、库存余额、入库单、出库单、库存流水、库存调整 | 暂不做库位、批次、序列号 |
| 系统权限模块 | 用户、角色、部门、权限码、登录鉴权 | 暂不做完整 IAM 平台 |
| AI 模块 | RAG 问答、业务查询 Tool、审计日志、分析工作流预留 | 暂不允许 AI 直接写业务数据 |

## 7. 核心业务主链路

项目第一阶段最重要的是保证采购、销售、退货和库存之间形成稳定闭环。
...
flowchart LR
    ...
    ST --> SO
    SO --> OUT["待确认出库单"]
    OUT --> FLOW_OUT["确认出库库存流水"]
    FLOW_OUT --> ST
    PO --> PR["采购退货单"]
    PR --> PR_OUT["待确认采购退货出库单"]
    PR_OUT --> ST
    SO --> SR["销售退货单"]
    SR --> SR_IN["待确认销售退货入库单"]
    SR_IN --> ST
...
- 采购订单审核后只生成待确认入库单，销售订单审核后只生成待确认出库单，均不直接改变库存。
- 采购退货审核后生成待确认出库单，销售退货审核后生成待确认入库单，均由仓库确认后才改变库存并回写退货单及退货明细处理进度；来源订单的累计已入库/已出库量保持不变，仅作为可退校验依据。
- 库存变动统一由仓库模块在确认入库单或出库单时处理。
- 库存流水承担已确认库存变动凭证，记录来源单据、出入库类型、操作人和确认时间。
- 库存余额用于快速查询，入库单、出库单和库存流水用于追溯作业进度与库存变化原因。
...
| `erp-warehouse` | 仓库、库存余额、入库单、出库单、库存流水和库存调整 |
| `erp-purchase` | 供应商、供货产品、采购订单、采购入库及采购退货来源能力 |
