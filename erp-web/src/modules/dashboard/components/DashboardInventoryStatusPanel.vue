    ...
    <div class="inventory-status-panel__body">
      ...
      <div v-if="canShow" :class="['inventory-status-panel__content', { 'inventory-status-panel__content--changing': transitioning }]">
        <section class="inventory-status-panel__chart" :aria-label="`${selectedWarehouseName}${TEXT.title}`">
          <div class="inventory-status-panel__donut" :style="donutStyle"><div><strong>{{ formatCount(riskCount) }}</strong><span>{{ TEXT.riskRecords }}</span></div></div>
        ...
        <section class="inventory-status-panel__preview">
          <div class="inventory-status-panel__preview-head"><div class="inventory-status-panel__preview-title"><span>{{ TEXT.riskRecords }}</span><small>{{ '\u5de5\u4f5c\u53f0\u4ec5\u5c55\u793a\u524d ' }}{{ PREVIEW_LIMIT }}{{ ' \u6761\u98ce\u9669\u8bb0\u5f55' }}</small></div><small class="inventory-status-panel__preview-count">{{ TEXT.preview }} {{ previewItems.length }}/{{ PREVIEW_LIMIT }}</small></div>
          <div v-if="previewItems.length" class="inventory-status-panel__table-wrap">
            <table class="inventory-status-panel__table">
              ...
              <thead><tr><th>{{ TEXT.product }}</th><th v-if="selectedWarehouseId === 'all'">{{ TEXT.warehouse }}</th><th>{{ TEXT.available }}</th><th>{{ TEXT.safety }}</th><th>{{ TEXT.status }}</th></tr></thead>
              <tbody><tr v-for="alert in previewItems" :key="alert.stockId"><td><div class="inventory-status-panel__product"><code>{{ alert.productCode }}</code><span :title="alert.productName">{{ alert.productName }}</span></div></td><td v-if="selectedWarehouseId === 'all'" :title="alert.warehouseName">{{ alert.warehouseName }}</td><td class="is-risk">{{ formatRiskQty(alert.availableQty, alert.quantityPrecision) }} {{ alert.unitName }}</td><td>{{ formatRiskQty(alert.safetyStockQty, alert.quantityPrecision) }} {{ alert.unitName }}</td><td><Badge variant="outline" :class="alertMeta(alert.severity).badge">{{ alertMeta(alert.severity).label }}</Badge></td></tr></tbody>
            </table>
          </div>
          <p v-else class="inventory-status-panel__empty">{{ TEXT.empty }}</p>
          <Button v-if="status?.riskPreview.hasMore" size="sm" variant="outline" class="inventory-status-panel__action" @click="openAllRisks">{{ TEXT.viewAll }}<ChevronRight class="ml-1 h-4 w-4" /></Button>
        </section>
      </div>
...
.inventory-status-panel__body { padding: 18px 20px 20px; }.inventory-status-panel__content { display: grid; grid-template-columns: minmax(360px, .90fr) minmax(0, 1.10fr); gap: 24px; transition: opacity 180ms var(--motion-ease-standard), transform 180ms var(--motion-ease-standard); }.inventory-status-panel__content--changing { opacity: .72; transform: translateY(2px); }
.inventory-status-panel__chart { display: grid; grid-template-columns: 200px minmax(0, 1fr); align-items: center; gap: 22px; min-height: 312px; padding: 24px; border: 1px solid color-mix(in srgb, var(--border) 75%, transparent); border-radius: 10px; background: color-mix(in srgb, var(--muted) 35%, var(--card)); }.inventory-status-panel__donut { display: grid; width: 200px; height: 200px; place-items: center; border-radius: 50%; transition: background 220ms var(--motion-ease-standard); }.inventory-status-panel__donut > div { display: grid; width: 136px; height: 136px; place-content: center; border-radius: 50%; background: var(--card); text-align: center; }.inventory-status-panel__donut strong { font-size: 38px; font-variant-numeric: tabular-nums; line-height: 1; }.inventory-status-panel__donut span { margin-top: 8px; color: var(--muted-foreground); font-size: 12px; line-height: 17px; }
.inventory-status-panel__legend { display: grid; width: fit-content; max-width: 100%; justify-self: center; grid-template-columns: 1fr; gap: 12px; margin: 0; padding: 0; list-style: none; }.inventory-status-panel__legend li { display: grid; grid-template-columns: 8px max-content 28px; align-items: center; gap: 8px; min-width: 0; color: var(--muted-foreground); font-size: 12px; }.inventory-status-panel__legend i { width: 8px; height: 8px; border-radius: 50%; }.inventory-status-panel__legend span { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.inventory-status-panel__legend strong { color: var(--foreground); font-variant-numeric: tabular-nums; }
.inventory-status-panel__preview { display: flex; min-width: 0; flex-direction: column; }.inventory-status-panel__preview-head { display: flex; align-items: flex-start; justify-content: space-between; gap: 12px; min-height: 35px; margin-bottom: 9px; color: var(--muted-foreground); font-size: 12px; font-weight: 600; }.inventory-status-panel__preview-title { display: grid; gap: 2px; }.inventory-status-panel__preview-head span { color: var(--foreground); font-size: 13px; }.inventory-status-panel__preview-title small { font-weight: 400; line-height: 16px; }.inventory-status-panel__preview-count { flex: 0 0 auto; padding-top: 1px; white-space: nowrap; }.inventory-status-panel__table-wrap { overflow-x: auto; border: 1px solid var(--border); border-radius: 9px; }.inventory-status-panel__table { width: 100%; min-width: 580px; border-collapse: collapse; table-layout: fixed; }.inventory-status-panel__table .product { width: 31%; }.inventory-status-panel__table .warehouse { width: 18%; }.inventory-status-panel__table .quantity { width: 14%; }.inventory-status-panel__table .state { width: 23%; }.inventory-status-panel__table .product-selected { width: 47%; }.inventory-status-panel__table th, .inventory-status-panel__table td { min-height: 48px; padding: 9px 10px; border-bottom: 1px solid var(--border); text-align: center; vertical-align: middle; font-size: 12px; line-height: 17px; }.inventory-status-panel__table th { height: 38px; background: color-mix(in srgb, var(--muted) 58%, var(--card)); color: var(--muted-foreground); font-weight: 600; }.inventory-status-panel__table tr:last-child td { border-bottom: 0; }.inventory-status-panel__product { display: grid; justify-items: center; gap: 4px; min-width: 0; }.inventory-status-panel__product code { max-width: 100%; overflow: hidden; border-radius: 4px; background: var(--muted); padding: 1px 4px; text-overflow: ellipsis; white-space: nowrap; font-size: 11px; }.inventory-status-panel__product span { display: -webkit-box; overflow: hidden; max-width: 100%; -webkit-box-orient: vertical; -webkit-line-clamp: 2; word-break: break-word; font-weight: 500; }.inventory-status-panel__table td:nth-child(2) { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.inventory-status-panel__table .is-risk { color: #be123c; font-weight: 650; font-variant-numeric: tabular-nums; }
.inventory-status-panel__action { align-self: flex-end; margin-top: 10px; }.inventory-status-panel__empty { display: flex; min-height: 180px; align-items: center; justify-content: center; gap: 8px; color: var(--muted-foreground); font-size: 13px; }.inventory-status-panel__preview > .inventory-status-panel__empty { min-height: 185px; }
@media (max-width: 1100px) { .inventory-status-panel__content { grid-template-columns: 1fr; }.inventory-status-panel__chart { grid-template-columns: 200px minmax(0, 1fr); min-height: 0; }.inventory-status-panel__legend { grid-template-columns: repeat(2, minmax(0, 1fr)); } }
@media (max-width: 720px) { .inventory-status-panel__header { align-items: stretch; flex-direction: column; }.inventory-status-panel__filter { flex-basis: auto; width: 100%; }.inventory-status-panel__body { padding: 14px; }.inventory-status-panel__chart { grid-template-columns: 1fr; justify-items: center; }.inventory-status-panel__legend { width: 100%; grid-template-columns: 1fr; }.inventory-status-panel__action { width: 100%; }.inventory-status-panel__table { min-width: 560px; } }
@media (prefers-reduced-motion: reduce) { .inventory-status-panel__content, .inventory-status-panel__donut { transition: none; } }
</style>