...
function nextMockReturnItemId() {
  return `2031000000000000${String(nextReturnItemSequence++).padStart(3, '0')}`;
}
...
function normalizeReturn(row: ReturnOrderListItem): ReturnOrderListItem {
  const returnType = assertEnum(row.returnType, ['SALES_RETURN', 'PURCHASE_RETURN'] as const, 'returnType');
  if (returnType !== 'PURCHASE_RETURN') throw new Error('采购退回接口不得返回其他退货类型');
  return {
    ...row,
...
async function getEligiblePurchaseDetails() {
  const pages = await Promise.all([
    listPurchaseOrders({ status: 'PARTIAL_INBOUND', pageNum: 1, pageSize: 20 }),
    listPurchaseOrders({ status: 'INBOUND_DONE', pageNum: 1, pageSize: 20 }),
  ]);
  const unique = new Map(pages.flatMap(page => page.records).map(order => [order.purchaseOrderId, order]));
  return Promise.all([...unique.keys()].map(id => getPurchaseOrderDetail(id)));
}
...
function buildSeed(
  ...
  const returnOrderId = `2030000000000000${String(index + 1).padStart(3, '0')}`;
  const returnNo = `PR202607${String(index + 1).padStart(3, '0')}`;
  const createTime = `2026-07-${String(10 + index).padStart(2, '0')} 09:30:00`;
  const returnItem: ReturnOrderItem = {
    returnOrderItemId: `2031000000000000${String(index + 1).padStart(3, '0')}`,
    returnOrderId,
    sourceOrderItemId: item.purchaseOrderItemId,
    ...
    availableReturnQty: item.inboundQty,
    requestedQty,
    approvedQty,
    processedQty,
    ...
    remark: index === 0 ? '供应商已确认接收，待提交审核' : '',
  };
  const order: ReturnOrderDetail = {
    returnOrderId,
    returnNo,
    returnType: 'PURCHASE_RETURN',
    sourceOrderId: source.purchaseOrderId,
    sourceOrderNo: source.purchaseNo,
    partyId: source.supplierId,
    partyCode: source.supplierCode,
    partyName: source.supplierName,
    ...
    returnReason: index % 2 === 0 ? '到货抽检发现质量异常' : '规格与采购约定不一致',
    totalAmount: 0,
    status,
    statusReason: status === 'CANCELLED' ? '供应商已补发，无需继续退货' : '',
    ...
    approvedByName: ['APPROVED', 'PARTIAL_EXECUTED', 'COMPLETED'].includes(status) ? '采购主管' : '',
...
async function ensureMockReturns() {
  ...
  const sources = await getEligiblePurchaseDetails();
  if (sources.length === 0) throw new Error('缺少可用于采购退回 Mock 的已入库采购单');
...
async function sourceDetail(sourceOrderId: string) {
  return getPurchaseOrderDetail(sourceOrderId);
}
...
async function mockSourceItems(sourceOrderId: string): Promise<ReturnableSourceOrderItem[]> {
  ...
  return source.items
    .filter(item => item.inboundQty > 0)
    .map(item => {
      const occupiedQty = occupiedQuantity(item.purchaseOrderItemId);
...
async function buildMockItems(returnOrderId: string, source: PurchaseOrderDetail, payload: ReturnOrderFormPayload, existingItems: ReturnOrderItem[] = []) {
  await ensureMockReturns();
  const sourceItems = await mockSourceItems(source.purchaseOrderId);
  return payload.items.map((draft): ReturnOrderItem => {
    ...
    if (!sourceItem) throw new Error('原采购明细已无剩余可退数量，请刷新后重试');
...
export async function listPurchaseReturns(query: ReturnOrderQuery): Promise<ReturnOrderPage> {
  const params = cleanListParams(query);
  if (!useMockApi) {
    const page = await getResult<ReturnOrderPage>('/purchase/returns', params);
  ...
  const filtered = rows.filter(row => {
    ...
    if (params.supplierId && row.partyId !== params.supplierId) return false;
...
export async function getPurchaseReturnDetail(returnOrderId: string): Promise<ReturnOrderDetail> {
  ...
  if (!useMockApi) return normalizeReturnDetail(await getResult<ReturnOrderDetail>(`/purchase/returns/${returnOrderId}`));
  const rows = await ensureMockReturns();
...
export async function createPurchaseReturn(payload: ReturnOrderFormPayload): Promise<ReturnOrderDetail> {
  const request: ReturnOrderCreateRequest = { ...payload, returnType: 'PURCHASE_RETURN' };
  if (!useMockApi) return normalizeReturnDetail(await postResult<ReturnOrderDetail, ReturnOrderCreateRequest>('/purchase/returns', request));
  const rows = await ensureMockReturns();
  const source = await sourceDetail(payload.sourceOrderId);
  const returnOrderId = `2030000000000000${String(nextReturnSequence).padStart(3, '0')}`;
  const returnNo = `PR${new Date().toISOString().slice(0, 10).split('-').join('')}${String(nextReturnSequence).padStart(3, '0')}`;
  ...
  const created: ReturnOrderDetail = {
    ...
    warehouseName: payload.warehouseId === source.warehouseId ? source.warehouseName : '退货出库仓库',
...
export async function updatePurchaseReturn(returnOrderId: string, payload: ReturnOrderUpdateRequest): Promise<ReturnOrderDetail> {
  if (!useMockApi) {
    const response = await http.put<Result<ReturnOrderDetail>>(`/purchase/returns/${returnOrderId}`, payload);
    return normalizeReturnDetail(response.data.data);
  }
  const rows = await ensureMockReturns();
  const row = rows.find(item => item.returnOrderId === returnOrderId);
  if (!row || row.status !== 'DRAFT') throw new Error('只有采购退回草稿可以编辑');
  if (row.version !== payload.version) throw new Error('数据已被其他人修改，请刷新后重试');
  const source = await sourceDetail(payload.sourceOrderId);
  row.sourceOrderId = source.purchaseOrderId;
  row.sourceOrderNo = source.purchaseNo;
  row.partyId = source.supplierId;
  row.partyCode = source.supplierCode;
  row.partyName = source.supplierName;
  row.warehouseId = payload.warehouseId;
  row.warehouseName = payload.warehouseId === source.warehouseId ? source.warehouseName : '退货出库仓库';
...
export async function deletePurchaseReturn(returnOrderId: string, version: number) {
  if (!useMockApi) {
    await http.delete(`/purchase/returns/${returnOrderId}`, { data: { version } });
    return;
  }
  const rows = await ensureMockReturns();
  const index = rows.findIndex(item => item.returnOrderId === returnOrderId);
  if (index < 0 || rows[index].status !== 'DRAFT') throw new Error('只有采购退回草稿可以删除');
...
export async function submitPurchaseReturn(returnOrderId: string, version: number) {
  if (!useMockApi) {
    await postResult<void>(`/purchase/returns/${returnOrderId}/submit`, { version });
    return;
  }
...
export async function approvePurchaseReturn(returnOrderId: string, payload: ReturnOrderApprovePayload) {
  if (!useMockApi) {
    await postResult<void>(`/purchase/returns/${returnOrderId}/approve`, payload);
    return;
  }
...
export async function rejectPurchaseReturn(returnOrderId: string, payload: ReturnOrderReasonActionPayload) {
  if (!useMockApi) {
    await postResult<void>(`/purchase/returns/${returnOrderId}/reject`, payload);
    return;
  }
  const row = await mockStatusRow(returnOrderId, payload.version, ['SUBMITTED']);
  row.status = 'DRAFT';
...
export async function cancelPurchaseReturn(returnOrderId: string, payload: ReturnOrderReasonActionPayload) {
  if (!useMockApi) {
    await postResult<void>(`/purchase/returns/${returnOrderId}/cancel`, payload);
    return;
  }
  const row = await mockStatusRow(returnOrderId, payload.version, ['DRAFT', 'SUBMITTED', 'APPROVED']);
  if (row.items.some(item => item.processedQty > 0)) throw new Error('已产生退货出库事实，不能取消');
...
export async function searchPurchaseReturnSourceOrders(keyword = ''): Promise<ReturnableSourceOrder[]> {
  const sourceOrderNo = keyword.trim();
  if (!useMockApi) {
    const page = await getResult<ReturnableSourceOrderPage>('/purchase/returns/source-orders', {
  ...
  const rows = await Promise.all(details
    .filter(order => !sourceOrderNo || order.purchaseNo.includes(sourceOrderNo))
    .map(async order => {
      const items = await mockSourceItems(order.purchaseOrderId);
      return normalizeSourceOrder({
        sourceOrderId: order.purchaseOrderId,
        sourceOrderNo: order.purchaseNo,
        partyId: order.supplierId,
        partyCode: order.supplierCode,
...
export async function listPurchaseReturnSourceItems(sourceOrderId: string): Promise<ReturnableSourceOrderItem[]> {
  ...
  const items = await getResult<ReturnableSourceOrderItem[]>(`/purchase/returns/source-orders/${sourceOrderId}/items`);
  return items.map(normalizeSourceItem);
}

export async function searchPurchaseReturnSupplierOptions(keyword = '') {
  const rows = await searchSupplierOptions(keyword, 10);
  return rows.map(row => ({ value: row.supplierId, label: `${row.supplierCode} ${row.supplierName}`, disabled: row.status === 0 }));
}

export async function searchPurchaseReturnWarehouseOptions(keyword = '') {
  return listEnabledWarehouseOptions(keyword, 10);
}
