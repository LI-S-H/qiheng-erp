      ...
      <DialogContent class="flex h-[min(720px,calc(100dvh-2rem))] max-h-[calc(100dvh-2rem)] flex-col overflow-hidden sm:max-w-3xl">
        ...
        <DialogScrollArea>
          <div class="grid grid-cols-2 gap-4 py-2 max-sm:grid-cols-1">
            ...
            <div class="space-y-1"><Label>最近采购价</Label><div class="relative"><span class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-sm text-muted-foreground">￥</span><Input v-model.number="form.latestPurchasePrice" type="number" min="0" step="0.01" class="pl-8" /></div><p v-if="formErrors.latestPurchasePrice" class="text-xs text-destructive">{{ formErrors.latestPurchasePrice }}</p></div>
