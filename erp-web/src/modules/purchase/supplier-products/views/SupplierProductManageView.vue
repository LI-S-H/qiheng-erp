...
const latestPurchasePriceInput = computed<string | number>({
  get: () => form.latestPurchasePrice ?? '',
  set: value => {
    form.latestPurchasePrice = value === '' ? null : Number(value);
  },
});
...
function cacheSupplierProductRow(row: SupplierProductListItem) {
  ...
  mergeProductOptions([{
    ...
    product: {
      ...
      referencePurchasePrice: row.latestPurchasePrice ?? 0,
    },
  }]);
}
...
const {
  ...
  load: fetchRecords,
  resetFilters: () => {
    Object.assign(query, { supplierId: 'all', productCode: '', productName: '', status: 'all' });
  },
});
...
function payload(): SupplierProductFormPayload {
  return {
    ...
    latestPurchasePrice: form.latestPurchasePrice === null ? null : Number(form.latestPurchasePrice),
      ...
      <DialogContent class="flex h-[min(720px,calc(100dvh-2rem))] max-h-[calc(100dvh-2rem)] flex-col overflow-hidden sm:max-w-3xl">
        ...
        <DialogScrollArea>
          <div class="grid grid-cols-2 gap-4 py-2 max-sm:grid-cols-1">
            ...
            <div class="space-y-1"><Label>产品 <span class="text-destructive">*</span></Label><RemoteSearchSelect :model-value="form.productId" :selected-label="selectedProductLabel" :fetch-options="fetchProductSearchOptions" placeholder="请选择产品" search-placeholder="输入产品编码或名称" :invalid="Boolean(formErrors.productId)" @update:model-value="syncProductPrice" /><p v-if="formErrors.productId" class="text-xs text-destructive">{{ formErrors.productId }}</p></div>
            <div class="space-y-1"><Label>供应商侧编码</Label><Input v-model="form.supplierProductCode" /><p v-if="formErrors.supplierProductCode" class="text-xs text-destructive">{{ formErrors.supplierProductCode }}</p></div>
            ...
            <div class="space-y-1"><Label>最近采购价</Label><div class="relative"><span class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-sm text-muted-foreground">￥</span><Input v-model="latestPurchasePriceInput" type="number" min="0" step="0.01" class="pl-8" /></div><p v-if="formErrors.latestPurchasePrice" class="text-xs text-destructive">{{ formErrors.latestPurchasePrice }}</p></div>
            <div class="space-y-1"><Label>最小起订量</Label><Input v-model.number="form.minOrderQty" type="number" min="0" :step="minOrderQtyStep()" /><p v-if="formErrors.minOrderQty" class="text-xs text-destructive">{{ formErrors.minOrderQty }}</p></div>
            <div class="space-y-1"><Label>预计交期天数</Label><Input v-model.number="form.leadTimeDays" type="number" min="0" step="1" /><p v-if="formErrors.leadTimeDays" class="text-xs text-destructive">{{ formErrors.leadTimeDays }}</p></div>
            <div class="space-y-1"><Label>推荐分</Label><Input v-model.number="form.aiScore" type="number" min="0" max="100" step="0.1" /><p v-if="formErrors.aiScore" class="text-xs text-destructive">{{ formErrors.aiScore }}</p></div>
            <div class="space-y-1"><Label>交付评分</Label><Input v-model.number="form.deliveryScore" type="number" min="0" max="100" step="0.1" /><p v-if="formErrors.deliveryScore" class="text-xs text-destructive">{{ formErrors.deliveryScore }}</p></div>
            <div class="space-y-1"><Label>质量评分</Label><Input v-model.number="form.qualityScore" type="number" min="0" max="100" step="0.1" /><p v-if="formErrors.qualityScore" class="text-xs text-destructive">{{ formErrors.qualityScore }}</p></div>
            <div class="space-y-1"><Label>价格评分</Label><Input v-model.number="form.priceScore" type="number" min="0" max="100" step="0.1" /><p v-if="formErrors.priceScore" class="text-xs text-destructive">{{ formErrors.priceScore }}</p></div>
      ...
      <DialogContent class="flex h-[min(680px,calc(100dvh-2rem))] max-h-[calc(100dvh-2rem)] flex-col overflow-hidden sm:max-w-4xl">
        ...
        <DialogScrollArea>
          <div v-if="detailRow" class="space-y-6 p-1">
            <BusinessDetailHero
              eyebrow="供应关系"
              :title="detailRow.productCode"
              :subtitle="`${detailRow.productName} · ${detailRow.supplierCode} · ${detailRow.supplierName}`"
            ...
            >
              <template #metrics>
                <div class="business-detail-hero__metric"><span>最近采购价</span><strong>{{ formatMoney(detailRow.latestPurchasePrice) }}</strong></div>
                <div class="business-detail-hero__metric"><span>预计交期</span><strong>{{ detailRow.leadTimeDays }} 天</strong></div>
                <div class="business-detail-hero__metric"><span>推荐分</span><strong>{{ formatScore(detailRow.aiScore) }}</strong></div>
                <div class="business-detail-hero__metric"><span>起订量</span><strong>{{ detailRow.minOrderQty }} {{ detailRow.unitName }}</strong></div>
              </template>
            </BusinessDetailHero>
            <BusinessDetailSection title="供货与采购条件" description="供应商、产品与采购标识。"><BusinessDetailFacts :columns="3"><div class="business-detail-fact"><dt>供应商</dt><dd><code>{{ detailRow.supplierCode }}</code><strong>{{ detailRow.supplierName }}</strong></dd></div><div class="business-detail-fact"><dt>产品</dt><dd><code>{{ detailRow.productCode }}</code><strong>{{ detailRow.productName }}</strong><small>单位：{{ detailRow.unitName }}</small></dd></div><div class="business-detail-fact"><dt>供应商侧编码</dt><dd><strong>{{ detailRow.supplierProductCode || '未维护' }}</strong></dd></div></BusinessDetailFacts></BusinessDetailSection>
            <BusinessDetailSection title="推荐与履约表现" description="展示供货推荐状态与历史履约评分。">
            ...
            <BusinessDetailSection title="采购记录与备注" description="最近采购记录与维护说明。"><BusinessDetailFacts :columns="3"><div class="business-detail-fact"><dt>最近采购时间</dt><dd><strong>{{ detailRow.lastPurchaseAt || '暂无' }}</strong></dd></div><div class="business-detail-fact"><dt>最后维护人</dt><dd><strong>{{ detailRow.updatedByName || '未记录' }}</strong></dd></div><div class="business-detail-fact"><dt>更新时间</dt><dd><strong>{{ detailRow.updateTime }}</strong></dd></div><div class="business-detail-fact"><dt>备注</dt><dd><strong>{{ detailRow.remark || '未维护' }}</strong></dd></div></BusinessDetailFacts></BusinessDetailSection>
          </div>
        </DialogScrollArea>
