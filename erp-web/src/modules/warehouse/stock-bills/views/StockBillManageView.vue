...
const pageText = computed(() => isAdjustmentPage.value ? {
  ...
  outboundLabel: '出库记录',
  sourceNoLabel: '来源单号',
  sourceNoPlaceholder: '如 PO202606001',
  toolbarTitle: '库存变动凭证',
  toolbarDescription: '业务单据正常生成凭证；遗漏登记时可手工补录，并保留负责人、原业务单号和补录原因',
  createButton: '新增出入库',
...
<template>
  <section class="page-shell space-y-4">
    ...
    <div class="summary-strip">
      <div class="summary-item"><span class="text-xs text-muted-foreground">{{ pageText.countLabel }}</span><strong class="mt-1 text-2xl">{{ summary.stockBillCount }}</strong></div>
      <div class="summary-item"><span class="text-xs text-muted-foreground">{{ pageText.inboundLabel }}</span><strong class="mt-1 text-2xl text-emerald-700">{{ summary.inboundCount }}</strong></div>
      <div class="summary-item"><span class="text-xs text-muted-foreground">{{ pageText.outboundLabel }}</span><strong class="mt-1 text-2xl text-blue-700">{{ summary.outboundCount }}</strong></div>
    ...
    <div class="data-panel relative">
      ...
      <div class="table-toolbar">
        <div class="table-toolbar__title"><strong class="text-sm">{{ pageText.toolbarTitle }}</strong><span class="text-xs text-muted-foreground">{{ pageText.toolbarDescription }}</span></div>
        <div class="table-toolbar__actions">
          <Tooltip><TooltipTrigger as-child><span class="inline-flex"><Button size="sm" :disabled="formSubmitting || actionSubmitting" @click="openCreateDialog"><Plus class="size-4" />{{ pageText.createButton }}</Button></span></TooltipTrigger><TooltipContent>{{ pageText.createTooltip }}</TooltipContent></Tooltip>
          <Tooltip><TooltipTrigger as-child><span class="inline-flex"><Button size="sm" variant="outline" :disabled="queryBusy" @click="refreshList">刷新</Button></span></TooltipTrigger><TooltipContent>{{ pageText.refreshTooltip }}</TooltipContent></Tooltip>
        </div>
      </div>
        ...
        <Table class="min-w-[1650px] table-fixed">
          ...
          <TableBody>
            ...
            <TableRow v-for="row in records" v-else :key="row.stockBillId" :data-stock-bill-id="row.stockBillId">
              ...
              <TableCell><div class="flex flex-col items-start gap-1"><div class="flex items-center gap-2"><span class="text-xs text-muted-foreground">{{ sourceTypeMap[row.sourceType] }}</span><span v-if="row.entryMode !== 'SOURCE_GENERATED'" data-manual-entry-marker class="inline-flex items-center gap-1.5 text-xs font-medium text-amber-700"><span class="size-1.5 rounded-full bg-amber-500" aria-hidden="true" />人工录入</span></div><code class="w-fit rounded bg-muted px-1.5 py-0.5 text-xs">{{ row.sourceNo || '无来源单号' }}</code></div></TableCell>
